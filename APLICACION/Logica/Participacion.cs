...
namespace APLICACION.Logica
{
    public static class Participacion
    {
        ...
        internal static DataTable filtrarPorFecha(string fecha)
        {
            ...
            string query = System.String.Format("select  CIUDAD.ciuCodigo AS \"CODIGO CIUDAD\", ciuNombre AS \"CIUDAD\", KARATECA.karatCodigo as \"CODIGO KARATECA\" , karatNombre AS \"NOMBE CARATECA\", karatNombre AS \"APELLIDO KARATECA\", karatPeso AS \"PESO KARATECA\", TORNEO.torCodigo AS \"CODIGO TORNEO\", torNombre AS \"NOMBRE TORNEO\", torModalidad AS \"MODALIDAD\", torFrecuencia AS \"FRECUENCIA\", TO_CHAR(partFecha, 'yyyy/mm/dd') AS \"FECHA\", TO_CHAR(partFecha, 'hh24:mi:ss') AS \"HORA\" from ciudad inner join karateca on ciudad.ciucodigo = karateca.ciucodigo inner join participa on participa.karatcodigo = karateca.karatcodigo inner join torneo on torneo.torcodigo = participa.torcodigo where TO_CHAR(participa.partFecha, 'dd/mm/yyyy') LIKE '{0}'", fecha);
